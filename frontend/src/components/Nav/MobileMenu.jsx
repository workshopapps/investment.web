...
const MobileMenu = ({ toggleMenu }) => {
    ...
    return (
        <div className='absolute top-0 bottom-0 right-0 left-0 backdrop-blur-sm bg-white/30'>
            <div className='absolute top-0 bottom-0 left-0 right-0 bg-transparent' onClick={() => toggleMenu(false)}>

            </div>
            <div className='absolute right-0 bg-white top-0 bottom-0 w-64'>
                <div className='py-6 px-6'>
                    <div className=' flex justify-end items-end mb-10' onClick={() => toggleMenu(false)}>
                    ...
                    <ul className="uppercase flex-col gap-4 w-full flex justify-center items-center mb-20">
                        {
                            navLinks.map((item, index) => {
                                const { link, url } = item;
                                return <MenuLink link={link} url={url} key={index}c />
                            })
                        }
                    </ul>
                    <div className=" justify-between items-center gap-4 nav-btns flex flex-col">
                        {btns.map((item, index) => {
                            ...
                            return <NavBtn name={name} background={background} color={color} key={index} />;
                        })}
                    </div>
                </div>
            </div>
