                        ...
                        {alphabets.map((letter, key) => {
                            switch (letter) {
                                case indexer?.toUpperCase():
                                    return (<span className="cursor-pointer border-b-2 border-[#1BD47B]" key={key}>{letter}</span>)
                                default:
                                    return (<span onClick={() => Router.push(`/companies?substring=${letter}`)} className="cursor-pointer" key={key}>{letter}</span>)
                            }
                        })}
                    </div>

                    <div className="grid md:grid-cols-2 lg:grid-cols-3 w-full my-10 gap-x-[15em] gap-y-5">
                        {data.map((company, key) => {
                            return (
                                <Link href={`/company/${company.ticker}`}>
                                    <p key={key}>{company.name}</p>
                                </Link>
                            )
                        })}
                    </div>
